data2 = data
data2 = which(is.na(data$Hypertension)==1)
mishype = data[data2,]
knn.cv(data_in,cl =data_in$Hypertension,k=3 , l = 2, prob = FALSE, use.all = TRUE)
#data[3501:4000,],



## Step 0  - Read in Data
data=read.csv("casestudydata(1).csv")
names(data)
...
data_in = data.frame(lapply(data_in, function(x)as.numeric(as.character(x))))
for(i in 1:ncol(data_in)){
  data_in[is.na(data_in[,i]), i] <- mean(data_in[,i], na.rm = TRUE)
}

# x <- mice(data_in, m = 5, method = NULL,where = NULL,
#      visitSequence = NULL, blots = NULL, post = NULL,
...
model5 = glm(CKD ~ Age + PVD + Hypertension + Diabetes + CHF +
               CVD + Anemia + Racegrpwhite , family="binomial" , data=data_in )
...
phat4=predict(model5,type="response")
summary(phat4)
...
summary(phat4)
classify=ifelse(phat4>.11,1,0)  # this is a threshold, we say if probability >50% , then say "yes"
summary(classify)  # notice that not many are "yes"  - is this desirable?
